...
@endphp
<form id="assignStockForm" method="POST" action="{{route('ic.postAssignStock',$param)}}">
	{{csrf_field()}}
	<div class="col-md-6">
		<div class="card">
			<div class="card-header bg-primary">
				...
				<strong> IC Assigned Stock</strong>
			</div>
			<div class="card-body custom-scrollbar" style="max-height: 500px;overflow-y: scroll;">
			...
			if(products)
			{
				...
				if(ids.length > 0)
				{
					...
					filteredProducts.forEach(product => {
						console.log(product)
						addRows += '<tr><td>'+product.name+'</td><td>'+product.category_id+'</td>'+
						'<td>'+product.brand_id+'</td><td>'+product.unit+'</td><td>'+product.quantity+'</td>'+
		...
		$('#draft').click(function(e){
			...
			then((value) => {
				if (value) {
					if($('#assignStockForm').parsley().validate())
					{
						var url ="{{route('ic.draftStock',$param)}}";
